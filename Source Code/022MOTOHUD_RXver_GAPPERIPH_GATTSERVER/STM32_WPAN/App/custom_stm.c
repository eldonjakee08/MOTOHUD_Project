...
/* Private typedef -----------------------------------------------------------*/
typedef struct{
  uint16_t  CustomEcu_Sensor_DataHdle;                    /**< ECU_SENSOR_DATA handle */
  uint16_t  CustomSensor_Data_BufferHdle;                  /**< SENSOR_DATA_BUFFER handle */
...
/* Private variables ---------------------------------------------------------*/
uint16_t SizeSensor_Data_Buffer = 32;
...
#define COPY_ECU_SENSOR_DATA_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
#define COPY_SENSOR_DATA_BUFFER_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   * service_max_attribute_record = 1 for ECU_SENSOR_DATA +
   *                                2 for SENSOR_DATA_BUFFER +
   *                              = 3
   *
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_SENSOR_DATA_BUFFER:
      ret = aci_gatt_update_char_value(CustomContext.CustomEcu_Sensor_DataHdle,
                                       CustomContext.CustomSensor_Data_BufferHdle,
                                       0, /* charValOffset */
                                       SizeSensor_Data_Buffer, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value SENSOR_DATA_BUFFER command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value SENSOR_DATA_BUFFER command\n\r");
      }
...
tBleStatus Custom_STM_App_Update_Char_Ext(uint16_t Connection_Handle, Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_SENSOR_DATA_BUFFER:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomEcu_Sensor_DataHdle, CustomContext.CustomSensor_Data_BufferHdle, SizeSensor_Data_Buffer, pPayload);
